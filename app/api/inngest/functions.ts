...
export const sendPeriodicJobListings = inngest.createFunction(
  ...
  async ({ event, step }) => {
          ...
          await step.run("send-email", async () => {
            const jobListingsHtml = recentJobs
              .map(
                ...
                <p style="margin: 5px 0;">${job.Company?.name || 'Unknown Company'} • ${job.jobProvince}</p>
            ...
            await resend.emails.send({
              from: "PharmaJob <onboarding@resend.dev>",
              to: ["thanatouchth@gmail.com"],
    ...
    return { userId, message: "Job listings sent" };
  }
);